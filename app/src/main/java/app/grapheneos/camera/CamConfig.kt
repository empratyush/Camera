...
//        }
    ...
    fun startCamera(forced: Boolean = false) {
        ...
        if (isQRMode) {
            ...
            if (isVideoMode) {
                ...
                if (!frameRates.isDefault()) {
                    Camera2Interop.Extender(videoRecorderBuilder).updateFrameRate()
        ...
        if (isVideoMode && !frameRates.isDefault()) {
            Camera2Interop.Extender(previewBuilder).updateFrameRate()
        }
    ...
    private fun Quality.toFrameRateKey(): String {
        return "${fromCameraxQualityToCamcorderQuality()}"
    }

    @OptIn(ExperimentalCamera2Interop::class)
    private fun <T> Camera2Interop.Extender<T>.updateFrameRate() {
